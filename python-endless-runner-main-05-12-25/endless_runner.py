...
MENU_BOX_WIDTH = GAME_WIDTH * 0.6  # Adjust this value for width
MENU_BOX_HEIGHT = 300  # Adjust this value for height
...
PAUSE_MENU_TITLE_PADDING = 20  # Adjust this value as needed
...
def draw_main_menu(surface):
    ...
    draw_text('Endless Runner', title_font, TEXT_SHADOW, surface, GAME_WIDTH / 2 + 2, GAME_HEIGHT / 4 + 2)
    draw_text('Endless Runner', title_font, WHITE, surface, GAME_WIDTH / 2, GAME_HEIGHT / 4)
...
def draw_pause_menu(surface):
    """Draws the pause menu with a static background box, 3D text, and hover effect."""
    draw_menu_background(surface)
    ...
    # Define specific font sizes for title and buttons
    title_font_size = 60  # Larger font size for the title
    button_font_size = BASE_FONT_SIZE  # Use the base font size for buttons
    ...
    # menu_box.fill( (0, 0, 0, 130))
    # surface.blit(menu_box, (box_x, box_y))

    mouse_pos = pygame.mouse.get_pos()

    # Draw the title with shadow and color
    ...
    button_rects = {}
    current_button_y = box_y + title_height + PAUSE_MENU_TITLE_PADDING + BOX_PADDING_Y;  # Use padding constant here
    ...
    for text in button_texts_list:
        ...
        # Draw button text with shadow and hover effect
        draw_text(text, font, TEXT_SHADOW, surface, rect.centerx + 2, rect.centery + 2)
        draw_text(text, font, TEXT_HOVER if is_hovered else WHITE, surface, rect.centerx, rect.centery)
...
def draw_instructions_screen(surface):
    ...
    draw_menu_background(surface)
    # Box dimensions for the pause menu
    PAUSE_MENU_BOX_WIDTH = 200  # Adjust this value for width (e.g., 60% of the game width)
    PAUSE_MENU_BOX_HEIGHT = 300  # Adjust this value for height (e.g., fixed height)
...
def draw_settings_screen(surface):
    ...
    # Back button
    button_back = pygame.Rect(GAME_WIDTH / 2 - 100, GAME_HEIGHT * 0.75, 200, 50)
    pygame.draw.rect(surface, WHITE, button_back)
    draw_text('Back', button_font, WHITE, surface, button_back.centerx, button_back.centery)
                ...
                if rect.collidepoint(mouse_pos):
                    if key == "resume":
                        ...
                        try:
                            ...
                            print(f"Error stopping music: {e}")
                        quit_game = True
                    break  # Prevents multiple actions
